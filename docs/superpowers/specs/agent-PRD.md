...
7. 高风险树结构变更必须先复述改写意图，再由用户语音确认。
8. MVP 只支持撤销最近一次已确认树操作，不支持 redo，也不支持新旧版本比较。
...
8. 若操作涉及高风险树结构变更，Orchestrator 生成复述改写文本并等待用户确认。
9. 用户确认后，Orchestrator 将每个 brief 发送给生图助理。
10. 生图助理返回图片和生成元数据。
11. Orchestrator 根据动作类型写树、写消息、写任务记录和树操作记录。
12. 前端收到更新并刷新画布和对话区。
